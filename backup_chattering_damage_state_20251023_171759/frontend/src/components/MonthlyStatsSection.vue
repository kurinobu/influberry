  ...
  <div v-if="!loading" class="monthly-stats-section bg-white rounded-b-lg shadow p-6">
    ...
    <div v-if="currentTab === 'overview'" class="overview-section">
      <div class="flex items-center mb-6">
        ...
        <h2 class="text-2xl font-bold text-gray-900">主要な実績</h2>
      </div>
      
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="stat-card bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-lg">
          <div class="text-sm text-blue-600 font-medium mb-2">累計活動案件数</div>
        ...
        <div class="stat-card bg-gradient-to-br from-green-50 to-green-100 p-6 rounded-lg">
          <div class="text-sm text-green-600 font-medium mb-2">累計入金額</div>
...
// プロジェクトデータの変更を監視して月次統計を自動更新（チャタリング防止のため一時無効化）
...
// 請求書データの変更を監視して月次統計を自動更新（チャタリング防止のため一時無効化）
...
// 月次目標データの変更を監視して月次統計を自動更新（チャタリング防止のため一時無効化）
...
// 月次切り替えチェック時刻の監視（チャタリング防止のため一時無効化）
