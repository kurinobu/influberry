...
export const useMonthlyRotationStore = defineStore('monthlyRotation', {
  ...
  actions: {
    ...
    /**
     * 手動で月次切り替えを実行（即座の状態同期機能付き）
     */
    async triggerManualRotation() {
      this.loading = true
      this.error = null
      
      console.log('🚀 手動月次切り替えを実行中...')
      ...
      try {
        ...
        if (response.data.success) {
          console.log('✅ バックエンドで月次切り替えが完了しました')
          ...
          // 即座のタブ再生成をトリガー
          console.log('🔧 即座のタブ再生成をトリガー中...')
          await this.triggerTabRegeneration()
          
          // フロントエンドデータの更新（修正追加）
          console.log('📊 フロントエンドデータを更新中...')
          await this.refreshFrontendData()
          
          console.log('🎉 手動月次切り替えが完了しました（即座の状態同期機能付き）')
          return true
        } else {
    ...
    async checkRotationStatus() {
      try {
        ...
        if (response.data.success) {
          ...
          // 月次切り替えが完了しているかチェック
          if (data.rotation_completed && data.snapshot_exists) {
          ...
          console.error('❌ 月次切り替え状態取得失敗:', response.data.error)
          return false
        }
      } catch (error) {
