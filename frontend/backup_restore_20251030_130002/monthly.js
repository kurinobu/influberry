...
export const useMonthlyStore = defineStore('monthly', {
  state: () => ({
    ...
    ,
    // 🔁 表示同期用の再レンダリングキー（保存直後の初回フレームnull回避）
    refreshKey: 0
  }),
    ...
    async saveTarget(targetMonth, data) {
      try {
        ...
        if (response.data && response.data.success) {
          ...
          // 3. データ同期の確実化（フレーム同期 + 再レンダーキー）
          await new Promise(resolve => setTimeout(resolve, 16))
          this.bumpRefreshKey()
          console.log('🔧 目標保存後: データ同期確実化完了（refreshKey進行）', { refreshKey: this.refreshKey })
