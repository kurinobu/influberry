...
const triggerManualRotation = async () => {
  try {
    const success = await rotationStore.triggerManualRotation()
    if (success) {
      alert('月次切り替えが完了しました。ページをリロードして確認してください。')
      // ページをリロード
      window.location.reload()
    } else {
      alert('月次切り替えの実行に失敗しました。')
    }
  } catch (error) {
    console.error('手動月次切り替えエラー:', error)
    alert('月次切り替えの実行中にエラーが発生しました。')
  }
}
</script>
        ...
        <div class="monthly-management-section mb-12" style="margin-bottom: 3rem !important;">
          ...
          <div class="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <div class="flex items-center justify-between">
              <div>
                <h3 class="text-sm font-medium text-yellow-800">開発用: 月次切り替えテスト</h3>
                <p class="text-xs text-yellow-600 mt-1">手動で月次切り替えを実行できます</p>
              </div>
              <button 
                @click="triggerManualRotation"
                :disabled="rotationStore.loading"
                class="px-3 py-1 bg-yellow-500 text-white text-sm rounded hover:bg-yellow-600 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {{ rotationStore.loading ? '実行中...' : '手動実行' }}
              </button>
            </div>
          </div>
        </div>
