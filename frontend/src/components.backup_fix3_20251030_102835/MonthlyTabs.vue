...
const checkCurrentState = () => {
  ...
  // nextTickの利用可能性を確認
  console.log('🔍 nextTick利用可能性:', typeof nextTick !== 'undefined')
  console.log('🔍 nextTick関数:', nextTick)
...
const selectNewMonthTab = async () => {
  ...
  try {
    ...
    if (rotationState === 'completed' && lastRotationCheck) {
      ...
      if (newMonthTab) {
        ...
        if (typeof nextTick !== 'undefined') {
          ...
          console.log('⚠️ nextTickが利用できません - 代替処理を実行')
          emit('update:modelValue', newMonthId)
        }
        ...
        if (monthlyTabs.length > 0) {
          ...
          if (typeof nextTick !== 'undefined') {
            ...
            // nextTickが利用できない場合の代替処理
            console.log('⚠️ フォールバック処理: nextTickが利用できません - 代替処理を実行')
            emit('update:modelValue', fallbackTab.id)
          }
    ...
    try {
      console.log('🔧 フォールバック処理: nextTickなしでタブ選択を実行')
      ...
      if (rotationState === 'completed' && lastRotationCheck) {
        ...
        console.log('⚠️ フォールバック処理: 月次切り替え未完了のためスキップ')
      }
    } catch (fallbackError) {
...
const switchToNewMonthTab = async (tabId) => {
  ...
  try {
    ...
    // ここでは追加の処理のみ実行
    console.log('🔧 emitの二重発行問題を解決: emitは既に実行済み')
