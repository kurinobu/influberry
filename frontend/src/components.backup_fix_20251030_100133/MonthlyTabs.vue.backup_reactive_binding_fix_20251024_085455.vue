...
const applyVisualChanges = () => {
  ...
  try {
    ...
    if (rotationState === 'completed' && lastRotationCheck) {
      ...
      // 3. 新しい月のタブをハイライト
      console.log('🎨 新しい月のタブをハイライト')
      
      // 4. 実行中のタブをアニメーション
      console.log('🎨 実行中のタブをアニメーション')
      
      // 5. 前月のタブをサブトル表示
      console.log('🎨 前月のタブをサブトル表示')
      
      // 6. DOM要素に視覚的変化を適用
      setTimeout(() => {
        const tabElements = document.querySelectorAll('.monthly-tabs button')
        console.log('🎨 DOM要素に視覚的変化を適用:', tabElements.length)
        ...
        tabElements.forEach((element, index) => {
          ...
          if (tab) {
            ...
            // 実行中のタブをアニメーション
              ...
              element.classList.add('shadow-lg', 'border-orange-300')
            ...
            // 通常のタブ
              ...
              element.classList.remove('animate-pulse', 'bg-gradient-to-r', 'from-green-50', 'to-blue-50')
              element.classList.remove('ring-2', 'ring-green-400', 'ring-opacity-50')
              element.classList.remove('animate-bounce', 'bg-orange-50')
              element.classList.remove('shadow-lg', 'border-orange-300')
              element.classList.remove('bg-yellow-50', 'opacity-75')
            }
          }
        })
        
        console.log('🎉 視覚的変化の適用完了')
      }, 100) // 100ms後に適用（DOM更新を待つ）
