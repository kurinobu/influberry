...
watch(() => props.invoice, (newInvoice, oldInvoice) => {
  console.log('Invoice watch triggered:', { newInvoice, oldInvoice })
  ...
  if (newInvoice) {
    ...
    // === Task 2: プロジェクト情報追加 ===
    formData.project_name = newInvoice.project_name || ''
  } else if (oldInvoice && !newInvoice && props.isOpen) {
    // 明確にnullになった場合のみ、かつモーダルが開いている場合のみクリア
    console.warn('Invoice data cleared - resetting form')
    resetForm()
  }
}, { immediate: true })
...
watch(() => props.isOpen, (isOpen) => {
  console.log('=== InvoiceForm props.isOpen変化 ===')
  console.log('新しいisOpen値:', isOpen)
  console.log('props.invoice:', props.invoice)
  console.log('=== InvoiceForm props確認完了 ===')
