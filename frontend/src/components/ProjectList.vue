...
const saveEdit = async () => {
  ...
  try {
    ...
    // 完了済みプロジェクトの場合はステータスのみ送信
    const currentProject = projectsStore.projects.find(p => p.id === editingProject.value)
...
<template>
  <div class="space-y-6">
    ...
    <!-- 新規作成モード（独立した条件） -->
    <div v-if="editingProject === 'new'" class="berry-card">
      <div class="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow duration-200 p-6 space-y-4">
...
.berry-action-button.invoice.invoice-disabled:hover {
  background: #f0fdf4;
  border-color: #bbf7d0;
  opacity: 0.8;
}
