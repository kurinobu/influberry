      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-6">
        ...
        <div class="space-y-2">
          ...
          <input
            ...
            type="text"
            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent transition-all duration-200"
            placeholder="例: Instagram投稿キャンペーン"
            :class="{ 'border-red-500': errors.project_name }"
          />
          <p v-if="errors.project_name" class="text-red-500 text-sm">{{ errors.project_name }}</p>
        </div>
        ...
        <div class="space-y-2">
          ...
          <textarea
            ...
            rows="3"
            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent transition-all duration-200 resize-none"
            placeholder="特記事項、連絡先、その他メモ..."
            :class="{ 'border-red-500': errors.notes }"
          ></textarea>
          <p v-if="errors.notes" class="text-red-500 text-sm">{{ errors.notes }}</p>
        </div>
...
const validateForm = () => {
  ...
  if (!formData.deadline) {
    ...
    if (deadlineDate < minDate) {
      errors.deadline = '納期は1年前以降の日付である必要があります'
      isValid = false
    }
  }
...
watch(() => props.project, (newProject) => {
  if (newProject) {
    ...
    formData.notes = newProject.notes || ''
