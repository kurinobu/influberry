...
const notificationActionText = ref('')
const notificationActionUrl = ref('')
...
const generatePDF = async (invoiceId) => {
  ...
  try {
    ...
    if (!result.success) {
      ...
      // エラー通知を表示
      showErrorNotification('PDF生成エラー', result.error)
    } else {
      trackPdfDownload('invoice')
      // 成功通知を表示
      showSuccessNotification('PDF生成完了', '請求書がPDFで発行されました。', '開く', result.pdfUrl)
    }
  } catch (error) {
    trackError('pdf_generation', error.message, 'InvoiceList')
    showErrorNotification('PDF生成エラー', error.message)
  } finally {
...
// 通知表示メソッド
const showSuccessNotification = (title, message, actionText = '', actionUrl = '') => {
  notificationType.value = 'success'
  notificationTitle.value = title
  notificationMessage.value = message
  notificationActionText.value = actionText
  notificationActionUrl.value = actionUrl
...
const showErrorNotification = (title, message) => {
  ...
  notificationTitle.value = title
  notificationMessage.value = message
  notificationActionText.value = ''
  notificationActionUrl.value = ''
  showNotification.value = true
}
