...
def get_monthly_stats(year, month):
    ...
    try:
        ...
        negative_changes = db.session.query(
            func.count(func.distinct(ProjectStatusHistory.project_id))
        ).join(Project).filter(
            Project.user_id == user_id,
        ...
        # 完了案件数（正負集計ロジック）
        # contracted → completed: +1
        # completed → contracted: -1
        positive_completed = db.session.query(
            func.count(func.distinct(ProjectStatusHistory.project_id))
        ).join(Project).filter(
            Project.user_id == user_id,
            ProjectStatusHistory.old_status == 'contracted',
        ...
        negative_completed = db.session.query(
            func.count(func.distinct(ProjectStatusHistory.project_id))
        ).join(Project).filter(
            Project.user_id == user_id,
        ...
        # 正負集計
        completed_projects = positive_completed - negative_completed
        ...
        income_rate = (sent_amount / target_data['income']) if target_data['income'] > 0 else 0
